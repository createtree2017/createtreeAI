...
export const storage = {
  ...
  // 사용자별 모든 항목 가져오기
  async getUserAllItems(userId: number) {
    // 음악 항목 가져오기
    const musicItems = await db.query.music.findMany({
    ...
    // 저장된 채팅 항목 가져오기
    const chatItems = await db.query.savedChats.findMany({
      where: eq(savedChats.userId, userId),
      orderBy: [desc(savedChats.createdAt)],
    });
    
    // 채팅 항목 변환
    const transformedChats = chatItems.map((item) => ({
      id: item.id,
      title: item.title || '저장된 대화',
      type: "chat" as const,
      url: `/chat?id=${item.id}`,
      createdAt: item.createdAt.toISOString(),
      isFavorite: false,
    ...
    // 모든 항목 결합 및 생성일 기준 정렬
    return [...transformedMusic, ...transformedImages, ...transformedChats].sort((a, b) => {
      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    });
  },
  
  // 사용자별 음악 항목 가져오기
  async getUserMusicItems(userId: number) {
    const musicItems = await db.query.music.findMany({
      where: eq(music.userId, userId),
      orderBy: [desc(music.createdAt)],
      with: {
  ...
  // 사용자별 이미지 항목 가져오기
  async getUserImageItems(userId: number) {
    const imageItems = await db.query.images.findMany({
      where: eq(images.userId, userId),
      orderBy: [desc(images.createdAt)],
      with: {
  ...
  // 사용자별 채팅 항목 가져오기
  async getUserChatItems(userId: number) {
    const chatItems = await db.query.savedChats.findMany({
      where: eq(savedChats.userId, userId),
      orderBy: [desc(savedChats.createdAt)],
    });
