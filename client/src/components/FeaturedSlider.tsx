        ...
        <div className="flex">
          {items.map((item) => (
            <Link 
              key={item.id} 
              href={item.href}
              className="relative min-w-full block aspect-[4/5] overflow-hidden rounded-2xl group"
            >
              {/* 이미지 배경 - 상단 3/4 부분 */}
              ...
              {/* 텍스트 콘텐츠 - 하단 1/4 부분 */}
              <div className="absolute bottom-0 left-0 right-0 h-1/4 bg-[#2C3E50] p-3 md:p-4">
